...
const handler = auth(async function POST(req) {
  ...
  if (req.method === "POST") {
    ...
    try {
      const data =
        await sql`SELECT id,parent_id,userid,name,begin_at,end_at,completed,isdeleted,items,COUNT(id) OVER() as taskscount FROM tasks WHERE isdeleted=false AND completed=false AND userid=${user_id} AND begin_at::date<=${
          day as string
        } GROUP BY id ORDER BY begin_at desc LIMIT ${limit as number} OFFSET ${
          offset as number
        };`;
