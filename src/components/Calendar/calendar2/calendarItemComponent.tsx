...
export const CalendarItemComponent = memo(
  ({ paramDay }: { paramDay: TCData }) => {
    ...
    if (paramDay.id && !paramDay.enabled && paramDay.isSelected) {
      return (
        ...
        >
          <motion.div
            ...
            className={`px-1 py-[3px] w-[34px] h-[34px] transition-all relative ${
              paramDay.isSelected
                ? "text-slate-100 hover:text-white hover:outline-slate-600 "
                : " text-slate-200 "
            } bg-slate-500 text-[1rem] rounded-full outline outline-[1px] outline-slate-400 outline-offset-2 border border-slate-300  font-sans hover:outline-2`}
          >
            {currDay}
            {paramDay.isSelected && (
              <div className="w-fit absolute left-[18px] bottom-0 text-slate-200 z-10 drop-shadow-[-1px_2px_0_rgba(0,0,0,1)] ">
      ...
      return (
        <div className="w-[34px] h-[34px] bg-slate-300 rounded-full text-slate-100 uppercase text-center p-1">
          {currDay}
        </div>
      );
    }
    ...
    return (
      ...
      >
        <motion.div
          ...
          } text-[1rem] rounded-full outline outline-[1px] outline-sky-400 outline-offset-2 border border-sky-300  font-sans hover:outline-2`}
        >
          {currDay}
          {paramDay.isSelected && (
            <div className="w-fit absolute left-[18px] bottom-0 text-green-300 z-10 drop-shadow-[-1px_2px_0_rgba(15,25,160,1)]">
              <DaySelect_SVG pWidth={18} pHeight={18} />
            </div>
          )}
        </motion.div>
